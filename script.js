...
const text = {
  ...
  no: {
    ...
    charades: "Charades 🎭",
    spy: "Spion 🕵️",
    imposter: "Impostor kunstner 🎨",
    mrwhiteTitle: "Mr White 🕵️‍♂️",
    qsTitle: "Spørsmål Spion ❓",
...
function startMrWhite(){
  ...
  if(!players||players<3){alert(text[currentLanguage].minPlayers);return;}
  mwPlayer=0;
  mwOddPlayer=Math.floor(Math.random()*players);
...
function startQuestionSpy(){
  ...
  if(!players||players<3){alert(text[currentLanguage].minPlayers);return;}
  qsPlayer=0;
  qsOddPlayer=Math.floor(Math.random()*players);
...
function startPsychologist(){
  ...
  if(!players||players<3){alert(text[currentLanguage].minPlayers);return;}
  psychPlayer=0;
  psychOddPlayer=Math.floor(Math.random()*players);
